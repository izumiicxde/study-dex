...
if (!GEMINI_API_KEY) {
  console.error(
    "Missing API key - ensure environment variables are properly configured",
  );
}
...
export default function AiAssistant() {
  ...
  const [transcriptionConfidence, setTranscriptionConfidence] =
    useState<number>(0);
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      toast.error(
        `Failed to process ${file.name} securely. Please try another file.`,
      );
    } finally {
  ...
  const processSpeechText = (text: string): string => {
    ...
    const processedSentences = sentences.map((sentence) => {
      ...
      if (!processed.match(/[.!?]$/)) {
        if (
          QUESTION_WORDS.some((word) =>
            processed.toLowerCase().startsWith(word),
          )
        ) {
          processed += "?";
        } else if (
          EXCLAMATION_INDICATORS.some((word) =>
    ...
    if ("webkitSpeechRecognition" in window && !recognition.current) {
      try {
        ...
        recognition.current.onerror = (event: SpeechRecognitionErrorEvent) => {
          ...
          const message =
            errorMessages[event.error] || "Speech recognition error occurred";
          toast.error(message);
  ...
  const saveTranscription = () => {
    ...
    try {
      ...
      const safeFileName = `studymate-notes-${timestamp}.txt`.replace(
        /[^\w.-]/g,
        "",
      );
            ...
            <div className="flex items-center space-x-2">
              ...
              {(transcribedText || finalTranscript) && !isRecording && (
                <button
                  ...
                  disabled={
                    isProcessingFile || !(transcribedText || finalTranscript)
                  }
                >
